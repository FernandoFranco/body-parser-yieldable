...
var yieldableParse = require('../lib/utils/yieldable-parser');
var bodyParserYieldable = require('..');

describe('bodyParserYieldable()', function () {
  ...
  describe('when JSON is invalid', function () {
    ...
    it('should 400 for bad token', function (done) {
      request(this.server)
        ...
        .set('Content-Type', 'application/json')
        .send('{:')
        .expect(400, this.badTokenError, done)
    })
    ...
    it('should 400 for incomplete', function (done) {
      request(this.server)
        ...
        .set('Content-Type', 'application/json')
        .send('{"user"')
        .expect(400, this.incompleteError, done)
    })
    ...
    describe('when undefined', function () {
      ...
      it('should 400 on primitives', function (done) {
        request(this.server)
          ...
          .set('Content-Type', 'application/json')
          .send('true')
          .expect(400, this.primitivesError.replace('#', 't'), done)
      })
    })
    ...
    describe('when true', function () {
      ...
      it('should not parse primitives', function (done) {
        request(this.server)
          ...
          .set('Content-Type', 'application/json')
          .send('true')
          .expect(400, this.notParsePrimitivesError.replace('#', 't'), done)
      })
      ...
      it('should not parse primitives with leading whitespaces', function (done) {
        request(this.server)
          ...
          .set('Content-Type', 'application/json')
          .send('    true')
          .expect(400, this.notParsePrimitivesWithLeadingWhitespacesError.replace('#', 't'), done)
      })
      ...
      it('should include correct message in stack trace', function (done) {
        request(this.server)
          ...
          .send('true')
          .expect(400)
          .expect(shouldContainInBody(this.includeCorrectMessageInStackTrace.replace('#', 't')))
          .end(done)
      })
    })
  })
  ...
  return http.createServer(function (req, res) {
    _bodyParser(req, res, function (err) {
      if (err) {
        ...
        const xErrorProperty = req.headers['x-error-property']
        res.end(err[xErrorProperty || 'message'])
      } else {
        res.statusCode = 200
...
function parseError (str) {
  return new Promise(function promise(resolve) {
    yieldableParse(str, undefined, undefined, function parse(err) {
